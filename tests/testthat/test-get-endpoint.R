...
test_that("get_endpoint descarta parámetros NULL antes de validar", {
  ...
  mock_perform <- mock(fake_resp)
  mock_body    <- mock(list(provincias = list(list(id = "06", nombre = "Buenos Aires"))))

  stub(get_endpoint, "httr2::req_perform",   mock_perform)
  stub(get_endpoint, "httr2::resp_body_json", mock_body)
...
test_that("get_endpoint emite warning cuando la respuesta está vacía", {
  ...
  stub(get_endpoint, "httr2::req_perform",    mock(structure(list(), class = "httr2_response")))
...
test_that("get_endpoint devuelve tibble con columnas esperadas para provincias", {
  ...
  stub(get_endpoint, "httr2::req_perform",    mock(structure(list(), class = "httr2_response")))
  stub(get_endpoint, "httr2::resp_body_json", mock(list(provincias = fake_data)))
